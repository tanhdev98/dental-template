$(document).ready(function () {
  ...
  document.querySelectorAll(".form-select-container").forEach((sc) => {
    ...
    option_placeholder.setAttribute(
      "data-default",
      sc.querySelector(".option-placeholder").textContent
    );

    if (option_placeholder.textContent === option_placeholder.getAttribute("data-default")) {
      option_placeholder.style.color = "#999";
    }
    ...
    Array.from(select_html.children)
      .filter((o) => {
        return !o.classList.contains("option-placeholder");
      })
      .forEach((o) => {
  ...
  document
    .querySelectorAll(".form-select-container > .form-select-cus > .form-option-placeholder")
    .forEach((f_o_p) => {
      f_o_p.addEventListener("click", openOptionList);
      f_o_p.addEventListener("blur", closeOptionList, true);
      f_o_p
        .closest(".form-select-cus")
        .querySelector(".form-option-wrapper")
        .querySelectorAll(
          ".form-option-placeholder-container > .form-option-placeholder, .form-option-container > .form-option"
        )
        .forEach((f_o) => f_o.addEventListener("click", closeOptionList));
    });
  ...
  function openOptionList(e) {
    ...
    let form_option_container = e.currentTarget
      .closest(".form-select-cus")
      .querySelector(".form-option-wrapper > .form-option-container");
  ...
  document
    .querySelectorAll(".form-select-container > .form-select-cus > .form-option-wrapper")
    .forEach((f_o_w) => {
      let form_options = f_o_w.querySelectorAll(
        ".form-option-placeholder-container > .form-option-placeholder, .form-option-container > .form-option"
      );
  ...
  function updateOption(e, options) {
    let form_option_placeholder = e.currentTarget
      .closest(".form-select-cus")
      .querySelector(".form-option-placeholder");
    ...
    if (
      form_option_placeholder.textContent === form_option_placeholder.getAttribute("data-default")
    ) {
      form_option_placeholder.style.color = "#999";
    } else {
      form_option_placeholder.style.color = "";
    }
